...
can readily be split into Hermitian $\mathcal{P}_H$ and
...
\begin{equation}
  ...
  -
  \mathcal{P}_A^*(x, m, k)
  \label{eq:PCIResponseIdentities:phase_factor_Hermitian_decomposed_symmetry}
\end{equation}

Note that the above symmetry also implies a degeneracy when $m = 0$.
Specifically,
(\ref{eq:PCIResponseIdentities:phase_factor_Hermitian_decomposed})
states that
\begin{equation}
  \mathcal{P}(x, +0, k)
  =
  ...
  \mathcal{P}_A(x, 0, k)
\end{equation}
while (\ref{eq:PCIResponseIdentities:phase_factor_Hermitian_decomposed_symmetry})
shows that
\begin{equation}
  \mathcal{P}(x, -0, k)
  =
  ...
  \mathcal{P}_A^*(x, 0, k)
\end{equation}
Of course, $\mathcal{P}(x, +0, k) = \mathcal{P}(x, -0, k)$; that is
\begin{equation}
  \mathcal{P}_H(x, 0, k)
  +
  \mathcal{P}_A(x, 0, k)
...
that is
\begin{align}
  \mathcal{P}_H(x, 0, k) &= [\mathcal{P}_H(x, 0, k)]^*
  \\
  \mathcal{P}_A(x, 0, k) &= -[\mathcal{P}_A(x, 0, k)]^*
\end{align}
This proves that $\mathcal{P}_H(x, 0, k)$ is purely \emph{real} and
that $\mathcal{P}_A(x, 0, k)$ is purely \emph{imaginary}.
