...
\begin{align}
  E_G(\vect{r_m})
  &=
  \mathcal{F}^{-1}[E_G(\vect{k}_{\perp}, z)](\vect{r_m})
  \notag \\
  &=
  \frac{1}{(2 \pi)^2}
  \int d\vect{k}_{\perp}
  e^{i \vect{k}_{\perp} \cdot \vect{r_m}}
  E_G(\vect{k}_{\perp}, z)
  \notag \\
  &=
  \frac{1}{(2 \pi)^2}
  ...
  E_G(\vect{r}')
  \int d\vect{k}_{\perp} \,
  e^{i \vect{k}_{\perp} \cdot (\vect{r_m} - \vect{r'})}
  \notag
\end{align}
where
$\vect{k}_{\perp} \equiv k_x \hat{\vect{x}} + k_y \hat{\vect{y}}$
is the perpendicular wavevector, and
$\vect{\rho}' = x' \hat{\vect{x}} + y' \hat{\vect{y}}$
is the perpendicular distance from the optical axis.
...
based upon perpendicular wavenumber $\vect{k}_{\perp}$;
if this manipulation can be described
in terms of a transfer function $T(\vect{k}_{\perp})$,
