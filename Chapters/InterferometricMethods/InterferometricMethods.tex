...
\subsection{The Fraunhofer diffraction pattern and the Fourier transform}
Examine again (\ref{eq:InterferometricMethods:Fraunhofer_diffracted_field}),
which gives the electric field in the Fraunhofer limit.
The field is proportional to $E_0 D_x D_y$.
Note that
\begin{equation}
  E_0 D_x D_y
  =
  \mathcal{F} \left[ E(\vect{\rho}', z=0) \right](\vect{k}_{\perp}')
  \label{eq:InterferometricMethods:Fraunhofer_diffraction_relation_to_Fourier_transform}
\end{equation}
where $\mathcal{F}$ is the Fourier transform operator,
$E(\vect{\rho}', z=0)$ is the field on surface $S_1$, and
\begin{equation}
  \vect{k}_{\perp}'
  \equiv
  \frac{k_0}{r} \vect{\rho}
  ...
  +
  k_y' \hat{\vect{y}}
  \label{eq:InterferometricMethods:k_perp_prime}
\end{equation}
is the wavenumber perpendicular to the optical axis.
Thus, knowledge of the field on surface $S_1$ can be used
to determine the Fraunhofer diffraction patterns $D_x$ and $D_y$
or vice versa.
In particular, if $D_x$ and $D_y$ are known,
the field at $S_1$ must be
\begin{equation}
  E(\vect{\rho}', z=0)
  =
  \mathcal{F}^{-1} \left[ E_0 D_x D_y \right](\vect{\rho}')
  \label{eq:InterferometricMethods:S1_field_from_Fraunhofer_diffraction_pattern}
\end{equation}

While
(\ref{eq:InterferometricMethods:S1_field_from_Fraunhofer_diffraction_pattern})
is trivial,
it gives a natural foundation for more sophisticated analysis.
For example, let $S_1$ sit at the object plane of an imaging system.
However, in between the object plane and the image plane,
imagine that the Fraunhofer field is somehow manipulated;
if this manipulation can be described
in terms of a transfer function $T(\vect{k}_{\perp}')$, then
the \emph{effective} object-plane field $E_{\text{eff}}$
is no longer $E(\vect{\rho}', z=0)$ but is instead
\begin{equation}
  E_{\text{eff}}(\vect{\rho}', z=0)
  =
  \mathcal{F}^{-1} \left[%
    T(\vect{k}_{\perp}') \cdot E_0 D_x D_y
  \right](\vect{\rho}')
  \label{eq:InterferometricMethods:effective_object_plane_field}
\end{equation}
This effective object-plane field will be used in
Section~\ref{sec:InterferometricMethods:pci:wavenumber_response}
to determine the wavenumber response
of a phase contrast imaging (PCI) system.
...
and its width $w(z_j)$ radius of curvature $R(z_j)$
...
\begin{equation}
  \mathcal{I}[E_G(\vect{r_m})] e^{-i (\omega_0 + m \omega) t}
  =
  [E_{G'}(\vect{r', t})]
  e^{i (m \xi - m^2 \kappa)}
...
\end{align}
corresponds to the undiffracted Gaussian beam in the image plane and
\begin{align}
  \xi
  &=
  \frac{k x'}{M} - \omega t
  \label{eq:InterferometricMethods:xi_imaged_wave_phase}
  \\
  \kappa
  &=
  \frac{1}{2 k_0 R'(z')} \left( \frac{k x'}{M} \right)^2
  \label{eq:InterferometricMethods:kappa_image_plane_curvature_effect}
\end{align}
The curvature-induced phase factor $\kappa$ is typically negligible and
is dropped in the remainder of this chapter's discussion.
\graffito{\textcolor{red}{%
  But $\kappa$ can become important during calibrations.
  Cite Naoto and Dorris. Or even save for later chapter?}}
...
Most detectors of interest are square-law detectors
...
\begin{align}
  I(\vect{r}, t)
  &\equiv
  \frac{c \varepsilon_0}{2} |\mathcal{I}[E(\vect{r}, t)]|^2
  \notag \\
  &=
  I_{G'}(\vect{r'})
...
if the detector is only exposed to the imaged probe beam.
...
While the phase plate is typically located
at the focal plane of a focusing optic,
the radiation pattern in the focal plane
is equivalent to the far-field radiation pattern.
As the far-field, diffracted electric field is already known from
(\ref{eq:InterferometricMethods:Fraunhofer_phase_modulated_Gaussian_beam_diffraction}),
the following calculations will be performed in the far field.
It should be noted that the far-field radiation pattern
is simply the Fourier transform of the field
with wavevectors $k_x = k_0 x / r$ and $k_y = k_0 y / r$.
Let $\vect{k_{\perp}} = k_x \hat{\vect{x}} + k_y \hat{\vect{y}}$.
Then, the far-field electric field can be written as
\begin{equation}
  \begin{aligned}
    E(\vect{k_{\perp}}, z, t)
    =
...
\begin{equation}
  ...
  \right]
\end{equation}
The effect effect of the phase plate then is to alter
the far-field radiation pattern as
\begin{equation}
  E_{\text{PCI}}(\vect{k_{\perp}}, z, t)
  =
  T_{\text{pp}}(k_x)
  \cdot
  E(\vect{k_{\perp}}, z, t)
\end{equation}
\graffito{\textcolor{red}{Note that taking inverse Fourier transform
  effectively transforms from far-field back to object plane}}
Taking the inverse Fourier transform in the $x$-direction yields
\begin{align}
  E_{\text{PCI}}(\vect{r}, t)
  =
  \mathcal{E}
  ...
  \right\}
\end{align}
where $\mathcal{F}_x^{-1}$ is
the inverse Fourier transform operator
in the $x$-direction.
For notational simplicity, define the phase-plate operator $\mathcal{P}$
\begin{equation}
  \mathcal{P}(x, k, m)
  \equiv
  \mathcal{F}_x^{-1}
  \left\{
    ...
    e^{-\left[ \frac{w_0}{2} \left( k_x - m k \right) \right]^2}
  \right\}
  \label{eq:InterferometricMethods:phase_plate_action_on_mth_beam}
\end{equation}
Several useful properties of $\mathcal{P}(x, k, m)$ are derived in
