...
only the lowest order $m = 0, \pm 1$ terms contribute substantially to
(\ref{eq:InterferometricMethods:imaged_total_field}).
Noting that $J_{-m}(z) = (-1)^m J_m(z)$~\cite{abramowitz_and_stegun},
the total scattered field in the image plane is
\begin{equation}
  \mathcal{I}[ E(\vect{r}, t) ]
  \approx
  E_{G'}(\vect{r'}, t)
  \left\{%
    J_0(\tilde{\phi}_0)
    +
    2 i [J_1(\tilde{\phi}_0)] \cos \left( \frac{k x'}{M} - \omega t \right)
  \right\}
\end{equation}
Further, the Bessel function's small-argument limiting form is
\cite{abramowitz_and_stegun}
\begin{equation}
...
Thus, the total scattered field in the image plane further simplifies to
\begin{equation}
  \mathcal{I}[ E(\vect{r}, t) ]
  \approx
  E_{G'}(\vect{r'}, t)
