...
For all the cases considered in this work,
$\mathcal{E}_G(\vect{r_m}) = E_G(\vect{r_m}) \cdot \mathcal{P}(\vect{r_m})$
for some complex-valued function $\mathcal{P}$ such that
the imaging optics simply transform
the $m$\ts{th} diffracted beam as
\begin{equation}
  \mathcal{I} \left[ \mathcal{E}_G(\vect{r_m}) \right]
  =
  E_{G'}(\vect{r_m'}) \cdot \mathcal{P}(\vect{r_m'})
\end{equation}
where
\begin{equation}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
and $w'(z_m')$, $R'(z_m')$, and $\psi'(z_m')$ are the beam's image-plane
...
\begin{equation}
  \mathcal{I}[\mathcal{E}_G(\vect{r_m})] e^{-i (\omega_0 + m \omega) t}
  =
  E_{G'}(\vect{r', t})
  \cdot
  \mathcal{P}(\vect{r_m'})
...
\begin{equation}
  ...
  \biggl\{%
    ...
    \left[
      \mathcal{P}(\vect{r'}_1) e^{i \zeta}
      +
      \mathcal{P}(\vect{r'}_{-1}) e^{-i \zeta}
    \right]
  \biggr\}
  \end{aligned}
  \label{eq:InterferometricMethods:imaged_total_field_weak_coupling_Fourier_filtered}
\end{equation}
where the substitution $\vect{r'}_0 = \vect{r'}$ has been used.
When there is no wavenumber-dependent manipulation of the diffracted beams
(i.e.\ $T(\vect{k}_{\perp}) = 1$ or, correspondingly,
$\mathcal{P}(\vect{r_m'}) = 1$ for all $m$),
the imaged field further simplifies to
\begin{equation}
  \mathcal{I}[ E(\vect{r}, t) ]
  =
  E_{G'}(\vect{r'}, t)
