...
of the phase fluctuations that are responsible for the scattering.
...
is imaged as $k / M$, and
the coordinate system of the $m$\ts{th} scattered beam in the image plane
is rotated by angle $\theta_m / M$
relative to the optical axis of the unscattered beam;
that is, in the image plane
\begin{align}
  \vect{k_m'}
  &=
  \mathcal{I}(\vect{k_m})
  ...
  \mathcal{I}(\vect{r_m})
  =
  \left( x - \frac{z \theta_m}{M},\, y,\, z + \frac{x \theta_m}{M} \right)
  \label{eq:InterferometricMethods:coordinate_rotation_image_plane}
\end{align}
In (\ref{eq:InterferometricMethods:coordinate_rotation_image_plane})
note that $x$ and $y$ refer to the image plane's transverse coordinates
relative to the optical axis of the unscattered Gaussian beam,
whereas $z$ refers to the axial distance
between the image plane and the waist of the unscattered Gaussian beam ---
in general, the beam waist does \emph{not} sit at the image plane
(i.e.\ $z \neq 0$).

As hinted at above, the imaging optics additionally alter
the properties of the incident Gaussian beams.
In addition to its in-medium wavelength $\lambda_0 / N$,
a Gaussian beam is fully specified by
its width $w(z_i)$ and
...
the nominal axis of the optical train
