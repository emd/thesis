...
a reference phase that is constant in time $(d\phi_R / dt = 0)$.
For a given equilibrium phase $\bar{\phi}$,
the amplitude of the fluctuating signal is maximized when
$\phi_R - \bar{\phi} = \pi / 2 + m \pi$ for integer $m$.
For concreteness, take $\phi_R - \bar{\phi} = \pi / 2$,
...
\end{equation}
Physically, $\phi_R - \bar{\phi} = \pi / 2$ means that the reference beam is
...
\graffito{\textcolor{red}{Modify for arbitrary $\phi_R - \bar{\phi}$}}
\begin{align}
  P_j(t)
  &=
...
from the assumption that $\phi_R - \bar{\phi} = \pi / 2$.
However, this is much easier said than done.
First, a CO$_2$ beam passing through $l \sim \SI{1}{\meter}$
of a plasma of density $n_e \sim \SI{e20}{\per\meter\cubed}$
...
Finally, the homodyne-measured fluctuation power $\tilde{P}_j$
...
As in the homodyne configuration,
finite sampling-volume effects~\cite{bravenec_rsi95} also dictate
...
\begin{align}
  P_{j,\,\text{het}}(t)
  &=
  \int_{\text{det}_j} I_{\text{het}}(\vect{r}, t) dA
  \notag \\
  &\begin{aligned}
    \approx
    2 I_G(\vect{r_j}) s_y
