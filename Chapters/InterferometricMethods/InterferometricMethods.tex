...
\subsection{The imaged field and the phase-plate transfer function}
...
complex valued.
However, for \emph{perfect} imaging,
...
It is useful to examine both the low-$k$ and high-$k$ limits
of $T_{\text{pp}}$ to ensure that they agree with expectations.
As $k \rightarrow 0$, the PCI response is expected to decrease
as the scattered beam falls into the phase-plate groove
and the phase contrast is lost, and
the response should fully vanish for $k = 0$.
Eqs.
...
state that
$\lim_{k_{\image} \rightarrow 0} F_1 = F_0$ and
$\lim_{k_{\image} \rightarrow 0} G_1 = G_0$, respectively.
Thus,
\begin{align}
...
\begin{align}
  ...
  \notag
\end{align}
where the third line in the limiting form of $C_Q$
follows from the fact that
$F_0$ is purely real and $G_0$ is purely imaginary.
As $C_I$ and $C_Q$ vanish at $k = 0$,
$C$ and $T_{\text{pp}}$ must also vanish at $k = 0$,
in agreement with expectations.
In the high-$k$ limit ($k \gg k_g$),
as long as the unscattered beam is well-within the phase-plate groove
(i.e.\ $k_g \gtrsim 2 / w_{0,\object}$),
$F_0 \approx 0$, $G_0 \approx i \sqrt{\eta}$,
$F_1 \approx 1$, and $G_1 \approx 0$ such that
$|T_{\text{pp}}| \approx 2 / \sqrt{\eta}$ and $\theta \approx 0$,
as is expected from
(\ref{eq:InterferometricMethods:pci_intensity}).
