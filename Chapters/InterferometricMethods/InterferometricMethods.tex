...
The effective object-plane field is then given by
(\ref{eq:InterferometricMethods:effective_object_plane_field})
with $D_x$ given by
(\ref{eq:InterferometricMethods:Fraunhofer_diffraction_integral_phase_modulated})
and $D_y$ given by
(\ref{eq:InterferometricMethods:Fourier_transform_free_space_Gaussian},
with the substitution $x \rightarrow y$); specifically,
\begin{align}
  E_{\text{eff}}(\vect{\rho}', z=0)
  &=
  \mathcal{F}^{-1} \left[%
    T_{\text{pp}}(k_x') \cdot E_0 D_x D_y
  \right](\vect{\rho}')
  \notag \\
  &=
  E_0
  \left\{ \mathcal{F}^{-1} \left[ D_y \right](y') \right\}
  \left\{ \mathcal{F}^{-1} \left[%
    T_{\text{pp}}(k_x') \cdot D_x
  \right](x') \right\}
  \notag \\
  &\begin{aligned}
    &=
