...
Typically, $\theta_m \ll 1$ such that
the far-field limit ($z \gg x$) implies that $z_m \gg x_m$ and
\begin{align}
  z &\approx z_m
  \\
  x &\approx x_m + z_m \theta_m
  \label{eq:InterferometricMethods:coordinate_rotation_transverse_dimension}
\end{align}
Substituting
(\ref{eq:InterferometricMethods:coordinate_rotation_transverse_dimension})
into
(\ref{eq:InterferometricMethods:Fraunhofer_phase_modulated_Gaussian_beam_diffraction})
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Note that the bracketed expression has the form of
(\ref{eq:InterferometricMethods:Fraunhofer_Gaussian_beam_diffraction})
for a far-field Gaussian beam such that
the diffracted electric field can be more compactly written as
\begin{equation}
  E(\vect{r}, t)
  \approx
  \sum_{m = -\infty}^{\infty}
...
\end{equation}
where $\vect{r_m} \equiv (x_m, y, z_m)$.
