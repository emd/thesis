...
$\mathcal{E}(\vect{r}_{m, \image}, k_{\image})$
readily reduces to $x$-dependencies of
the Gaussian envelope and phase-front curvature
of the unscattered ($m = 0$) beam:
\begin{align}
  \mathcal{E}(\vect{r}_{m, \image}, k_{\image})
  &=
  \exp\left[\frac{-x_{m, \image}^2}{w(z_{m, \image})^2} \right]
  \exp\left\{%
    i \left[%
      \frac{k_0 x_{m, \image}^2}{2 R(z_{m, \image})}
    \right]
  \right\}
  \notag \\
  &\approx
  ...
  \exp\left\{%
    i \left[%
      \frac{k_0 x_{\image}^2}{2 R(z_{\image})}
    \right]
  \right\},
  \label{eq:InterferometricMethods:mth_diffracted_beam_no_kx_filter_transformation}
\end{align}
...
Substituting
(\ref{eq:InterferometricMethods:mth_diffracted_beam_no_kx_filter_transformation})
into
...
$\tilde{I}_{\text{pci}}(\vect{r}_{\image}, t) \propto \cos \nu$.
...
the complex-valued phase-fluctuation images $e^{i \nu_j}$
