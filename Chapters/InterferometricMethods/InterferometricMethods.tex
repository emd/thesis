...
\graffito{\textcolor{red}{What about bulk plasma phase $\bar{\phi}$?}}
\begin{equation}
  E(\vect{r}, t)
  =
  E_G(\vect{r}, t)
...
\begin{equation}
  ...
  =
  2 I_G(\vect{r})
  (1 + \cos\phi_R + \tilde{\phi}_0 \sin\phi_R \cos\xi)
...
A constant reference phase $(d\phi_R / dt = 0)$ allows homodyne detection.
The amplitude of the fluctuating signal is then maximized when
$\phi_R = \pi / 2 + m \pi$ for integer $m$.
For concreteness, take $\phi_R = \pi / 2$,
for which the intensity on the detector becomes
\begin{equation}
  I(\vect{r}, t)
  =
  2 I_G(\vect{r})
  (1 + \tilde{\phi}_0 \cos\xi)
  \qquad
  \text{for} \quad
  \phi_R = \frac{\pi}{2}
\end{equation}
Physically, $\phi_R = \pi / 2$ means that the reference beam is
...
that produces the quadratic intensity variation in
...
Let the $j$\ts{th} detector element be centered on $x_j$ and $y = 0$;
the total power $P_j$ incident on this element is then
\begin{align}
  P_j(t)
  &=
  \int_{\text{det}_j} I(\vect{r}, t) dA
  \notag \\
  &\approx
  2 I_G(\vect{r_j}) s_y
  \int_{x_j - s_x / 2}^{x_j + s_x / 2}
  (1 + \tilde{\phi}_0 \cos\xi) dx
  \notag \\
  &=
  2 I_G(\vect{r_j}) A
  \left[%
    %1 + \tilde{\phi}_0 \cos\xi_j \sinc\left( \frac{k}{k_c} \right)
    1 + \tilde{\phi}_0 \sinc( k / k_c ) \cos\xi_j
  \right]
...
\begin{equation}
  ...
  \label{eq:InterferometricMethods:nomralized_sinc}
\end{equation}
...
\end{equation}
is the first zero (i.e.\ ``cutoff'')
in the fluctuating power's amplitude,
$\xi_j = (k x_j / M) - \omega t$, and
$A = s_x s_y$ is the area of the detector element.
The approximation in the above computation results
from assuming that the intensity profile $I_G(\vect{r})$
varies weakly over any given detector element
such that the intensity can be approximated as constant.
...
Note that the homodyne optical power in
...
$P_j(t) = \bar{P}_j + \tilde{P}_j(t)$, where
\begin{align}
  \bar{P}_j
  &=
  2 I_G(\vect{r_j}) A
  ...
  &=
  \bar{P}_j
  \tilde{\phi}_0 \sinc( k / k_c ) \cos\xi_j
\end{align}
Because $\tilde{\phi}_0 \ll 1$, $I_{\text{max}} \approx 2 I_G(0)$,
where $I_G(0)$ is the peak intensity of the undiffracted Gaussian probe beam.
Optimal performance of the system then requires that
$I_G(0) \approx I_{\text{sat}} / 2$.
To make ``apples-to-apples'' comparisons of different interference schemes,
it is useful to examine the ratio of fluctuating to equilibrium powers;
for homodyne interferometry, this ratio is
\begin{equation}
  \frac{\tilde{P}_j(t)}{\bar{P}_j}
  =
  T_{\text{hom}}(k) \cdot \tilde{\phi}_0 \cos\xi_j
\end{equation}
where
\begin{equation}
  T_{\text{hom}}(k)
...
all is not rosy in the land of the homodyne interferometer.
Recall that the above discussion follows
from the assumption that $\phi_R = \pi / 2$.
However, this is much easier said than done.
Fusion experiments are often characterized
by large, pulsed electromagnets
that can produce vibration-induced path-length changes.
A path-length change $\delta l$ produces
a corresponding reference-phase change $\delta \phi_R = k_0 \delta l$,
...
is sufficient to produce $\delta \phi_R \sim \pi / 2$,
...
actively controlled mirrors have been used
in an attempt to cancel vibration-induced path-length changes,
minimizing excursions from $\phi_R \approx \pi / 2$
...
Further, the homodyne-measured fluctuation power $\tilde{P}_j$
is \emph{not} an absolute measurement
...
can alter the equilibrium power $\bar{P}_j$;
this, in turn, alters the amplitude of the fluctuating power $\tilde{P}_j$.
Thus, it is generally impossible to distinguish
whether changes in the amplitude of the homodyne-measured $\tilde{P}_j$
