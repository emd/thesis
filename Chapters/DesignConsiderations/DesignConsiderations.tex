...
as shown in \textcolor{red}{Fig.~blah blah blah}.
...
\end{equation}
where $A_{2n - 1}$ is
the Fourier coefficient of the $(2n - 1)\ts{th}$ harmonic.
Typically, $A_{2n - 1}$ decreases as $n$ increases.
Then,
following Section~\ref{sec:DesignConsiderations:demodulation:ideal}'s program
of mixing the LO with the IF and low-pass filtering yields
\begin{align}
...
The coefficient $A_n / A_1$ is referred to as
...
\begin{equation}
  ...
  =
  20 \log_{10} \left( \frac{A_n}{A_1} \right).
\end{equation}
...
\begin{itemize}
  ...
  \item DC offset in baseband can result if diodes are not carefully matched,
    as the alternating virtual grounds are not equivalent
\end{itemize}
