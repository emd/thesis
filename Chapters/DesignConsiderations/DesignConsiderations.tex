...
\begin{itemize}
  ...
  \item \emph{Differences in the radii of curvature}
    ...
    These detrimental curvature effects will be negligible if
    \begin{equation}
      \frac{k_0}{8}
      \left[ (N_{\text{el}} s_x)^2 + s_y^2 \right]
      \left| \frac{1}{R_P(z_{\image})} - \frac{1}{R_R(z_R)}\right|
    ...
    for more details.
  \item \emph{Finite sampling-volume effects}
    dictate the wavenumber response of a heterodyne interferometer.
...
computed from the demodulated in-phase ($I$) and quadrature ($Q$) signals via
\begin{equation}
  \phi_m = \atantwo\left(Q, I\right),
\end{equation}
where $\atantwo\left(Q, I\right)$
