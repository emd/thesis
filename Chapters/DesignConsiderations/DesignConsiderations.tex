...
\begin{align}
  % \delta P_I(t) + i \cdot \delta P_Q(t)
  \delta P_{IQ}(t)
  =
  e^{- i \Delta \omega_0 t} \cdot \delta P(t).
\end{align}
Here, the demodulated noise has not yet been low-pass filtered.
...
the autospectral density of the detector noise.
The demodulated signals are typically low-pass filtered
such that only the desired information at $|\omega| \ll \Delta \omega_0$
survives
\begin{equation}
  \left.
    S_{\delta P_{IQ},\delta P_{IQ}}(\omega)
...
\end{equation}
which follows from the realness of $\delta P$
(i.e.\ $S_{\delta P, \delta P}(-\omega) = S_{\delta P, \delta P}(\omega)$).
Thus, the detector noise at the heterodyne frequency $\Delta \omega_0$
...
\begin{equation}
  \left.
    G_{\delta P_{IQ},\delta P_{IQ}}(\omega)
  \right|_{|\omega| \ll \Delta \omega_0}
  \approx
  \cdots
\end{equation}
\graffito{\textcolor{red}{relate autospectral density to NEP}}
...
\begin{align}
  ...
  \hbar \omega_0 P_0,
\end{align}
\graffito{\textcolor{red}{correct? Also, what is $P_0$?}}
