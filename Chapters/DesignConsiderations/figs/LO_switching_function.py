...
class LO(object):
    def __init__(self, Fs=360., tlim=[0, 3], phase_shift=0):
        ...
        # Signal
        self.x = 2 * np.cos((2 * np.pi * self.t) + phase_shift)
...
if __name__ == '__main__':
    ...
    axes[0].set_ylabel('LO [AU]', fontsize=fontsize)
    axes[0].set_ylim([-3, 3])
    ...
    axes[1].set_xlim([LO_I.t[0], LO_I.t[-1]])
    axes[1].set_ylim([-2, 2])
    axes[1].set_xlabel('$t \; [2 \pi / \Delta \omega_0]$', fontsize=fontsize)
    axes[1].set_ylabel('switching function', fontsize=fontsize)

    # Manage tick marks
    xticks = np.arange(np.int(LO_I.tlim[0]), np.int(LO_I.tlim[1]) + 1, 1)
    yticks0 = np.array([-2, 0, 2])
    yticks1 = np.array([-1, 0, 1])
