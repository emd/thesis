...
\cite[Sec.~8.3]{wesson}.
Relative to the ITG,
the ETG spatial scales in a $T_e \sim T_i$ deuterium plasma
...
The experiment was run in H-mode with the ITER-similar shape
(i.e.\ aspect ratio, elongation, and triangularity
...
the lab-frame phase velocity $\vph = 2 \pi f / k$,
where $f$ is the lab-frame frequency and
$k$ is the wavenumber,
it can be seen that the lowest frequencies
in a particular fluctuation branch
...
\begin{figure}
  ...
  \caption[PCI wavenumber spectra]{%
...
Note that the PCI spectrum
also depends on $k_{\rho,0}$ and $\Delta \vect{k}$,
as discussed in Section~\ref{sec:TurbulenceMeasurements:Modeling:wavevectors},
so quantitative comparison with the PCI measurements
requires a full-physics simulation
(i.e.\ beyond the abilities of a reduced model like TGLF) and
a synthetic diagnostic~\cite{rost_pp10}.
It should also be emphasized that the TGLF-SAT$1$ model
...
\begin{figure}
  \centering
  \includegraphics[width = \textwidth]{%
    Chapters/TurbulenceMeasurements/figs/density_spectra_local_ky_rhos.pdf}
  \caption[TGLF-predicted electron-density fluctuation spectra]{%
    ...
    the equivalent noise floor intersects $S(k_y)$
    at $k_y \rho_s \approx 2.7$, and
    $k_y \rho_s \approx \SI{2.7}
    \leftrightarrow
    k_R \approx \SI{15}{\per\centi\meter}$ at $\rho = 0.6$,
    which is where the corresponding $S(k_R)$
    intersects the noise floor
    in Figure~\ref{fig:TurbulenceMeasurements:Sk_power_law}.
    Within the PCI sensitivity and wavenumber domain,
    the predicted spectra are qualitatively consistent
    with the PCI-measured spectra
    ...
    as discussed in Section~\ref{sec:TurbulenceMeasurements:Modeling:wavevectors},
    so quantitative comparison with the PCI measurements
    requires a full-physics simulation and a synthetic diagnostic.
  }
