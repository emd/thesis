...
$k_{||} v_{ti} \ll \omega \ll k_{||} v_{te}$~\cite[Sec.~8.3]{wesson},
...
$v_{tj}$ is the thermal speed of species $j$;
...
\begin{itemize}
  \item ETG
  \item Multiscale coupling predictions
    \begin{itemize}
      \item Howard -- C-Mod L-mode and ELMy H-mode
      \item Insufficiency of reduced mass ratio simulations
      \item Holland -- \diiid\space IBS ELMy H-mode
    \end{itemize}
  \item Multiscale experiment
\end{itemize}

ETG
\begin{itemize}
  \item Passing particle instability similar to ITG but with
    roles of electrons and ions reversed (?)
  \item $k_{\perp} \rho_i \gg 1$
  \item Spatiotemporal scale? Drive?
\end{itemize}
...
TEM
\begin{itemize}
  \item When mode frequency $\omega_j$
    exceeds particle bounce frequency $\omega_{bj}$,
    trapping can typically be neglected
    for species $j$~\cite[Sec.~8.2]{wesson}
  \item Passing particles effectively average out their parallel velocity
    over a bounce period, reducing the importance of parallel dynamics
    relative to passing instabilities~\cite[Sec.~8.4]{wesson}
  \item Trapped particles also do not sample the full poloidal and toroidal
    circumferences, remaining in the region of bad magnetic curvature,
    introducing magnetic mirror-like instabilities
    \cite[Sec.~8.4]{wesson}
  \item TEM is dissipative, requiring collisions, but not
    so frequent as to prevent the trapped particles
    from completing a banana orbit~\cite[Sec.~8.4]{wesson}
  \item Mode frequency $\omega \approx \omega_{*e}$ with
    main drive being electron temperature gradient
    \cite[Sec.~8.4]{wesson}
  \item Spatiotemporal scale? Drive?
\end{itemize}
...
MTM
\begin{itemize}
  \item Electromagnetic and can form small-scale magnetic islands,
    with similarities to macroscopic tearing modes but with
    short wavelength and high poloidal mode number $m$
    \cite[Sec.~8.5]{wesson}
  \item Standard tearing mode theory (resistive MHD)~\cite[Sec.~7.3]{wesson}
    predicts stability due to high poloidal mode number
    causing $\Delta'$ to be negative (stabilizing);
    however, two additional effects compete and can
    result in growth
    \begin{itemize}
      \item Nonlinear aspects that compete with linear $\Delta'$
      \item Kinetic effects driven by electron temperature gradient
        in a sufficiently collisional plasma
    \end{itemize}
  \item Stability in semi-collisional regime:
    electron flow velocity $v_{||}$ along field lines
    limited by collisions with ions;
    competition between field-line bending $\Delta'$ and
    electron gradient drive $\eta_e$
  \item Expansion in $\omega_{*e} / \nu_{ei}$,
    so invalid at low collisionality but
    no collisions removes thermal force and stability expected;
    MTM peaks at intermediate $\nu_{ei}$
  \item Nonlinear effects important when width of magnetic island
    exceeds the ``resistive layer''\ldots
  \item Nonlinear drift effects again depend on $\eta_e$
  \item ``\ldots identification of this regime is rather subtle.''
  \item Spatiotemporal scale? Drive?
\end{itemize}
...
were performed with the TGLF code.
