...
that results from mixing the LO and IF.
Low-pass filtering each signal is performed
with a Mini-Circuits BLP-$10.7$+, which
has $\leq \SI{1}{\decibel}$ insertion loss
over its DC to $\SI{11}{\mega\hertz}$ passband.
The resulting $I$ and $Q$ signals are each
$-18 \; \text{dBm}$ (i.e.\ $\SI{28.2}{\milli\volt}$ RMS).

\textcolor{red}{Schematic?}


\subsection{Audio amplifiers}
\label{sec:Implementation:Hardware:audio_amps}
$\SI{50}{\ohm}$ termination of I\&Q prior to audio amps
...
\begin{itemize}
  ...
  \item atan$2$
  \item Filtering with zero-delay, Kaiser-designed FIR filter
\end{itemize}<++>
