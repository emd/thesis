...
Physical limits:
\begin{itemize}
  \item $I_{\text{sat}} = \SI{100}{\milli\watt\per\square\milli\meter}$
  \item $I_{\text{dam}} = \SI{1000}{\milli\watt\per\square\milli\meter}$
...
Story:
\begin{itemize}
  \item Obtain reasonable balance between AC \& DC components, but
    relatively low absolute level ($\ll I_{\text{dam}}$, but
    $\sim I_{\text{sat}}$)
  \item Can't push higher due to imposed floor on $\mathcal{P}_{\text{pci}}$
  \item Can get around this by cooling detector to increase sensitivity
  \item However, TE cooling imposes cooling limit
    $Q_{\text{heat}} \lesssim \SI{75}{\milli\watt}$, which
    is what constrains interferometer performance
    \emph{at the detector} in practice
    \begin{itemize}
      \item \ldots but how important is this in the full-system performance?
    \end{itemize}
  \item Methodology in practice: $\eta_P$ fixed by selection of splitter, and
    $\eta_R$ is increased slowly until interferometer thermal lock is
    barely held
\end{itemize}
