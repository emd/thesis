...
Predicted noise:
\begin{itemize}
  \item Autospectral density of demodulated detector noise
    (\ref{eq:DesignConsiderations:summary:detector_noise_autospectral_density})
    and autospectral density of demodulated optical shot noise
    (\ref{eq:DesignConsiderations:summary:shot_noise_autospectral_density})
    both depend on the probe-beam and reference-beam optical powers
    impinging on the detector element, $P_P$ and $P_R$, respectively.
  \item Referencing
    (\ref{eq:Implementation:Gaussian_beam_power_square_aperture}),
    ...
    Fig.~\ref{fig:Implementation:power_distribution}.
  \item Similarly the reference-beam optical power
    ...
    the design point indicated in
    Fig.~\ref{fig:Implementation:power_distribution}.
  \item Demodulated detector noise additionally depends on
    specific detectivity (\ref{eq:Implementation:Dstar}) of detector module
  \item Clearly, demodulated detector noise is predicted to be
    the largest contribution to the system noise,
    with quantization noise, OCXO phase noise, and
    demodulated optical shot noise sitting roughly
    one, two, and three orders of magnitude
    below the predicted detector noise.
\end{itemize}

Measured noise:
\begin{itemize}
  \item Realized quantization noise measured by keeping $I$ and $Q$ channels
    of digitizer \emph{open}; equivalent results obtained when terminating
    with $\SI{50}{\ohm}$. Note that
    realized quantization noise approximately $2 \times$ larger
    than predicted detector noise and approximately $20 \times$ larger
    than the predicted quantization noise.
    In practice, aperture error, jitter, and nonlinearities
    ...
    relative to predictions by a factor of two to four;
    however, the $20 \times$ factor seems a bit excessive\ldots
    The explanation for this anomalously high bit noise is
    not known, and it was not pursued any further.
  \item All electronics correspond to LO self-demodulation
    with OCXO, appropriate LO and IF levels, and
    all remaining electronics (demodulator, audio amps, etc.)
    configured as they are during operations.
  \item Full system includes laser, detector, and vibrations.
    Unknown if coherent peaks result from laser phase noise
    due to unmatched beam paths or vibrations.
    Establishes the noise floor at
    $\sim \SI{e-10}{\radian\squared\per\kilo\hertz}$
\end{itemize}
