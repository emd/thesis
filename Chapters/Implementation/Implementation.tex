...
its effect on the measurements of a heterodyne interferometer
should be measured.
...
however, without knowledge of the corresponding spectral distribution,
...
\begin{itemize}
  \item
  \item For example, the initial LO source was specified to be
    $0.01\%$ quartz stabilized, but without knowledge
    of the corresponding bandwidth, it is impossible
    to make even an order-of-magnitude estimate of phase noise.
  \item $\sim \SI{1.5}{\centi\meter}$ distance between
    center of AOM BNC port (taken as nominal position of transducer)
    and the optical axis of the AOM
  \item With $\SI{5400}{\meter\per\second}$ sound-wave speed in Ge,
    the corresponding coupling delay is $\tau \approx \SI{2.8}{\micro\second}$
  \item From the sine weighting of the LO phase noise in
    (\ref{eq:DesignConsiderations:summary:LO_phase_noise_autospectral_density}),
    the interferometer spectrum should exhibit ``ringing'' with
    nulls at harmonics of $1 / \tau \approx \SI{380}{\kilo\hertz}$;
    in fact, the interferometer spectrum exhibited ringing
    at harmonics of $\SI{400}{\kilo\hertz}$,
    indicating an actual coupling time $\tau = \SI{2.5}{\micro\second}$
\end{itemize}
\begin{figure}
  \centering
  \includegraphics[width = \textwidth]{%
