...
The idea was to as simply as possible implement interferometer detection
while minimally perturbing the operation of the PCI.


\subsection{Geometric effects of umatched beams}
...
\begin{equation}
  \begin{aligned}
    ...
    \bigr],
  \end{aligned}
  \label{eq:Implementation:heterodyne_intensity}
\end{equation}
where
\begin{equation}
  \bar{\phi}_{\text{eff}}
...
Note that (\ref{eq:Implementation:heterodyne_intensity}) readily reduces to
(\ref{eq:InterferometricMethods:heterodyne_intensity})
if $E_{G,R}(\vect{r}_R) = E_{G,P}(\vect{r}_{\image})$.

It is worth discussing the implications of heterodyne intensity
(\ref{eq:Implementation:heterodyne_intensity}).
...
\subsection{Phase noise from umatched optical path lengths}
The external reference-beam interferometry derivations
in Section~\ref{sec:InterferometricMethods:interferometry}
...
\graffito{\textcolor{red}{Would be great to get down to just one $\tau$}}
To see this, assume that the total optical path length
of the probe (reference) beam is $L_P$ ($L_R$)
such that the probe (reference) beam propagates
from the source to the detector
in time $\tau_P = L_P / c$ ($\tau_R = L_R / c$).
Then, the imaged probe radiation
impinging on the detector at time $t$ is
\begin{equation}
  E_P(\vect{r}_{\image}, t)
  =
  E_G(\vect{r}_{\image})
  e^{-i [\omega_0 (t - \tau_P) + \phi_{\omega_0}(t - \tau_P)]}
  e^{i \bar{\phi}}
  \left[%
    1
    +
  ...
  \right],
\end{equation}
and the corresponding reference beam is
\graffito{\textcolor{red}{assuming ref beam generation very near source}}
\begin{equation}
  E_R(\vect{r}_{\image}, t)
  =
  E_G(\vect{r}_{\image})
  e^{-i [
    (\omega_0 + \Delta \omega_0) (t - \tau_R)
    +
    \phi_{\omega_0}(t - \tau_R)
  ]}.
\end{equation}
Define the jitter-induced phase difference
between the reference beam and the probe beam as
\begin{equation}
  \delta \phi_{\omega_0}(t - \tau_P, t - \tau_R)
  =
  \phi_{\omega_0}(t - \tau_R)
  -
  \phi_{\omega_0}(t - \tau_P).
\end{equation}
Typically, $\delta \phi_{\omega_0} \ll 1$.
...
\subsection{LO frequency drift}
\subsection{Detector amplitude noise}

\begin{itemize}
  \item Ideal interferometer signals derived in Ch.~2
  \item If reference beam and plasma beam have different shapes,
    simply results in a geometric factor (?)
    \begin{itemize}
      \item Assume spot size is much larger than detector element;
        motivate by considering an array (even though we just have
        ``proof of principle'' with one detector element)
      \item Quote ``general'' formula (assuming radii of curvature matched)
      \item Define an effective intensity $I_{\text{eff}}$, and
        by comparing to Ch.~2 formula, simply extend all results
        from Ch.~2 by appropriately substituting $I_{\text{eff}}$
    \end{itemize}
  \item Effects of noise
    \begin{itemize}
      \item Assume detector noise (amplitude noise, more or less?)
      \item Assume ideal demodulation, audio amps, etc.
      \item Phase noise from laser (mention also from RF, but assume
        negligible for the design study)
    \end{itemize}
\end{itemize}
\subsection{Signal detection and processing}
\subsection{Simplifying and constraining the design parameters}
\subsection{Expected plasma-induced phase shift}
\subsection{Noise equivalent power}
\subsection{Expected signal-to-noise ratio}
\subsection{Power sharing between the interferometer arms and PCI}
\subsection{Optical table layout}
\begin{figure}
  \centering
  \includegraphics[width = \textwidth]{%
    Chapters/Implementation/figs/table_layout.pdf}
  \caption[Combined PCI-interferometer optical table]{%
    Combined PCI-interferometer optical table.
    Salmon: \emph{shared} probe beam;
    green: interferometer plasma arm;
    blue: interferometer reference arm;
    fuchsia: PCI beam;
    yellow: feedback beam.
    AOM: Ge acousto-optic modulator.
    BC: beam combiner.
    D1: LN$_{2}$-cooled PCI detector array;
    D2: room-temperature quadrant detector for beam steering;
    D3: thermoelectrically-cooled interferometer detector.
    L1, L2, L3: ZnSe lenses.
    P1, P2: off-axis parabolic mirrors.
    PP: phase plate.
    RM: rotatable mask.
    S1, S2: ZnSe beam splitters.
    SM: steering mirrors.
    $\lambda / 2$: half-wave plate.}
\label{fig:Implementation:table_layout}
\end{figure}
