...
An \emph{odd}-order, \emph{even}-symmetry,
finite-impulse-response (FIR) filter
has the desirable property that it produces zero delay
for all frequencies~\cite{kaiser_rsi77}.
Such a filter can be easily designed via
the Kaiser ``nearly equal ripple'' (NER) approximation
\cite{kaiser_rsi77,scipy_kaiser_window}.
Typically, such a Kaiser-designed, high-pass filter with
...
such a filter has an order $12489$
(corresponding to a $\sim \SI{3}{\milli\second}$ span), and
